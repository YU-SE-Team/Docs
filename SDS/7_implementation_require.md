...
| **서버 배포 형식** | Amazon에서 제공하는 클라우드 컴퓨팅 플랫폼인 AWS의 EC2를 활용하여 서버를 배포한다. 코드가 변경되어 푸시될 때마다 자동으로 빌드와 배포 과정을 수행할 수 있도록 GithubAction을 사용한다. 서버는 Docker를 이용해 컨테이너 형태로 배포한다. 이를 사용하면 환경까지 통째로 묶어서 배포할 수 있다. 데이터는 MySql을 통해 관리하며 안정적인 데이터베이스를 구축한다.
서버에는 NGINX를 설치해 웹 서버 및 리버스 프록시 역할을 수행하도록 한다. SSL 인증서를 적용하여 HTTPS 통신을 구축한다. 이를 통해 사용자와 서버 간 데이터가 암호화되어 안전하게 전송되도록 한다. |
